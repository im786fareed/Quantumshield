...
export default function HomePage() {
  const protectionFeatures = [
    ...
    { name: 'Device Scanner', icon: Smartphone, path: '/devicescan', color: 'text-teal-400' },
    { name: 'Evidence Vault', icon: Lock, path: '/evidence', color: 'text-emerald-400' },
    { name: 'Privacy Shield', icon: Shield, path: '/privacy', color: 'text-emerald-400' },
    ...
    <div className="min-h-screen bg-black text-white p-6 space-y-12 pb-24">
      ...
      <section>
        <div className="flex items-center gap-3 mb-8">
          <LayoutGrid className="text-emerald-400 w-6 h-6" />
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 gap-6">
          {protectionFeatures.map((f, i) => (
            <Link key={i} href={f.path} className="group bg-slate-900/40 border border-slate-800 p-8 rounded-[2.5rem] flex flex-col items-center text-center hover:border-emerald-500/50 transition-all">
      ...
      <section className="bg-emerald-600/10 border border-emerald-500/20 rounded-[3rem] p-12 text-center">
        <PlayCircle className="w-16 h-16 text-emerald-400 mx-auto mb-6" />
        <h2 className="text-3xl font-black uppercase tracking-tighter mb-4">Education Insight Center</h2>
        <Link href="/education" className="inline-block bg-emerald-600 px-12 py-4 rounded-2xl font-black shadow-xl">
